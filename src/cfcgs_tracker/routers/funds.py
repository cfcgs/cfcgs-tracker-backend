...
async def upload_fund_file(
    ...
    try:
        dataframe = read_file(file, file_type, upload_type=1)
...
@router.post(
    "/status", response_model=FundStatusResponse, status_code=HTTPStatus.OK
)
def status_of_funds(
    session: T_Session,
...
@router.patch(
    "/{fund_id}", response_model=FundSchema, status_code=HTTPStatus.OK
)
def update_fund(
    fund_id: int,
